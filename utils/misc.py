...
def construct_path(output_dir: str, exp_name: str) -> dict:
    ...
    str_datetime = str(datetime.now()).replace(":", "-")
    str_datetime = str_datetime.replace(" ", "-")

    tr_log_path = os.path.join(pth_log_path, f"tr_{str_datetime[:10]}.txt")
    te_log_path = os.path.join(pth_log_path, f"te_{str_datetime[:10]}.txt")
    trans_log_path = os.path.join(pth_log_path, f"trans_{str_datetime[:10]}.txt")
    cfg_copy_path = os.path.join(pth_log_path, f"cfg_{str_datetime}.py")
    trainer_copy_path = os.path.join(pth_log_path, f"trainer_{str_datetime}.txt")
